...
################
st.set_page_config(
    page_title="FIAM Vol Skew - Wavelet Transform Model",
    page_icon="🏄",
    layout="wide"
)
...
INTRO_XGB = "With the data de-noised, we train an XGBoost model on these approximation coefficients using an \
...
RAW_DATA_PATH = "/Users/akaashmitsakakis/Desktop/McGill-FIAM-Hackathon/data/oracle_costco_data.csv"
PREDICTED_RETURNS_PATH = "/Users/akaashmitsakakis/Desktop/McGill-FIAM-Hackathon/data/predicted_returns_wavelet_mp.csv"
...
def denoise_box_select_demo():
    ...
    feature = st.selectbox("Select a feature to investigate and de-noise across time with a DWT", map_dic.keys(), placeholder="Select a feature...")
    if feature:
        fig = hp.wavelet_demo(map_dic.get(feature), RAW_DATA_PATH)
        st.pyplot(fig)
        st.caption("By default, the company which has the most data points for that feature is chosen. Note also that the \
...
def tabular_predicted_df():
    ...
    model = st.selectbox("Select a model to display sample return predictions", model_dic.keys(), placeholder="Select a model...")
    if model:
